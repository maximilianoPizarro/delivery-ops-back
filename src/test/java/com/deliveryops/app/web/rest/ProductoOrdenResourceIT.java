package com.deliveryops.app.web.rest;

import static com.deliveryops.app.web.rest.TestUtil.sameNumber;
...
class ProductoOrdenResourceIT {
    ...
    @Mock
    private ProductoOrdenRepository productoOrdenRepositoryMock;

    @Mock
    private ProductoOrdenService productoOrdenServiceMock;
    ...
    void getAllProductoOrdensWithEagerRelationshipsIsEnabled() throws Exception {
        when(productoOrdenServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restProductoOrdenMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(productoOrdenServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllProductoOrdensWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(productoOrdenServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
